...
function ConnectionLines({ equipped }) {
  return (
    <svg style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
      ...
      {/* Solid active lines over dashed lines if equipped */}
...
export function ExoSkeletonWindow({ win, onUpdate }) {
  const [equipped, setEquipped] = React.useState(win.equipped || { head: null, mainHand: null, offHand: null, trinket: null });
  const [activeSlot, setActiveSlot] = React.useState(null); // 'head', 'mainHand', etc.

  const handleEquip = (slot, moduleId) => {
