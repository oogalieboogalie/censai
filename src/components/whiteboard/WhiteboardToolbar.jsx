    ...
    <div style={{
        ...
        {COLORS.map(c => (
          <button
            ...
            style={{
              ...
              borderRadius: '50%',
              background: c.value,
              border: color === c.value ? '2.5px solid var(--ink)' : '1px solid var(--hairline-strong)',
              cursor: 'pointer',
              padding: 0,
              boxShadow: color === c.value ? '0 0 6px var(--accent)' : 'none',
      ...
      <select
        ...
        onChange={(e) => {
          ...
          setStrokeWidth(w);
          if (selectedId) {
            queueSave(elements.map(el => el.id === selectedId ? { ...el, strokeWidth: w } : el));
          }
        }}
        style={{
      ...
      >
        {STROKE_WIDTHS.map(sw => (
          <option key={sw.value} value={sw.value} style={{ background: 'var(--surface-2)', color: 'var(--ink)' }}>{sw.label}</option>
        ))}
      </select>
      ...
      {selectedId && (
        <>
          ...
          <button 
            ...
            style={{ ...btnStyle, color: 'var(--ink-soft)' }}
          >
            ▲
          </button>
          ...
          <button 
            ...
            style={{ ...btnStyle, color: 'var(--ps-red)' }}
          >
            ✕
          </button>
        </>
