...
--  Durable artifacts, append-only workspace events, and typed edges.
-- ═══════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS artifacts (
  ...
  owner_kind TEXT NOT NULL DEFAULT 'agent'
    CHECK (owner_kind IN ('user', 'agent', 'system')),
  owner_id TEXT NOT NULL,
  visibility TEXT NOT NULL DEFAULT 'workspace'
    CHECK (visibility IN ('private', 'workspace', 'organization', 'public')),
...
CREATE INDEX IF NOT EXISTS idx_artifacts_workspace_type
  ON artifacts (workspace_id, artifact_type, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_artifacts_owner
  ON artifacts (owner_kind, owner_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS idx_artifacts_visibility
  ON artifacts (workspace_id, visibility);
CREATE INDEX IF NOT EXISTS idx_artifacts_metadata_gin
  ON artifacts USING GIN (metadata);
CREATE INDEX IF NOT EXISTS idx_artifacts_source_ref_gin
  ON artifacts USING GIN (source_ref);
...
CREATE INDEX IF NOT EXISTS idx_workspace_events_type
  ON workspace_events (workspace_id, event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_workspace_events_artifact
  ON workspace_events (artifact_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_workspace_events_correlation
  ON workspace_events (correlation_id);
CREATE INDEX IF NOT EXISTS idx_workspace_events_payload_gin
  ON workspace_events USING GIN (payload);
...
CREATE INDEX IF NOT EXISTS idx_relationships_source
  ON relationships (source_artifact_id, relationship_type);
CREATE INDEX IF NOT EXISTS idx_relationships_target
  ON relationships (target_artifact_id, relationship_type);
CREATE INDEX IF NOT EXISTS idx_relationships_workspace_type
