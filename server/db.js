...
export function createDbPool(connectionString = process.env.DATABASE_URL || 'postgresql://censai:censai@localhost:5432/censai') {
  let isLocal = true;
  try {
    if (connectionString.startsWith('postgresql://') || connectionString.startsWith('postgres://')) {
      const url = new URL(connectionString);
      const host = url.hostname;
      isLocal = host === 'localhost' || host === '127.0.0.1' || host === 'postgres' || host === 'censai-postgres';
    }
  } catch (e) {
